...
class Analysis_Neural_Net:
    ...
    def __init__(self, run_dir, numModels, all_vars, cont_vars, modes, data_dir, data_name, method_name, suffix=''):
        ...
        for index in range(self.numModels):
            ...
            model_file = os.path.join(self.run_dir, "{}_{}_model_{}.ckpt".format(data_name, method_name, str(index)))
            # check if files exist. If not, throw FileNotFoundError
            try:
                file_test = open(meta_graph_file, 'r')
                # file_test = open(model_file, 'r')
            except FileNotFoundError as e:
                print("Wrong file or file path")
                print(e.filename)

            self.list_meta_graph_file.append(meta_graph_file)
