...
class Analysis:
    """
    Reading in the prediction model (in pickle format) and calculating the economic metrics
    """
    def __init__(self, run_dir, numModels, all_vars, cont_vars, modes, data_dir, suffix=''):
        ...
        Args:
            ...
            cont_vars (list of str): the name list of continuous variables
            modes (list of str): the list of travel modes
            data_dir (str): the path of the model file (.pickle)
    ...
    def compute_prob_curve(self, x, input_x, var, prob, sess):
        ...
        choice_prob = []
        # prob_derivative is a list of np.array. Each element represents ...? 
        prob_derivative = []
    ...
    def load_models_and_calculate(self, mkt, disagg, social_welfare, drive_time_idx, drive_cost_idx, drive_cost_idx_standard, drive_time_name, drive_cost_name, drive_idx, time_correction):
        ...
        Args:
            ...
            drive_time_idx (int): the index of the drive time column in x
            drive_cost_idx (int): the index of the drive time column in x
            drive_cost_idx_standard ([type]): [description]
            drive_time_name ([type]): [description]
            drive_cost_name ([type]): [description]
            drive_idx ([type]): [description]
            time_correction ([type]): [description]
        """        
        self.filterd_train = []
        self.filterd_test = []
        sw_ind = []
